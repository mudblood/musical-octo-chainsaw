...
class="btn-floating">List something</button>
     </div>
...
console.log('🔥 On component init — loading should be true:', loading.value)

function onChatAfterEnter() {
  // Panel is now visible; ask child to focus its input.
...
//per-image loading  map
const imageLoadedMap = ref({})

const fetchListings = async () => {
  try {
    console.log('🚀 fetchListings started — setting loading to true')
    loading.value = true
    const res = await axios.get(`${API}/listings`)
    console.log('✅ fetchListings finished — response:', res)
    ...
    console.log('⏳ fetchListings ending — setting loading to false')
    loading.value = false
  }
}

const handleListed = async () => {
  console.log('📦 onMounted called — about to fetch listings')
  await fetchListings()
}
